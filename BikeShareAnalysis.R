...
train <- train %>%
  mutate(too_cold = if_else(temp < 2, 1, 0),
    ideal_temp = if_else(temp >= 2 & temp <= 33, 1, 0),
    too_hot = if_else(temp > 33, 1, 0) )
...
bike_recipe <- recipe(log_count~., data=train) %>% # Set model formula and dataset
  ...
  step_mutate(month = as.factor(lubridate::month(datetime)))%>%
